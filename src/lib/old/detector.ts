...
export async function detectLanguageFromHtml(
  html: string,
  ...
  if (script !== "Latn" && fallbackLanguageFromScript(script) === language)
    confidence += 0.2;
  if (htmlLang && francLang && htmlLang !== francLang) confidence -= 0.2;
  confidence = Math.max(0, Math.min(1, confidence));
  const needsConfirmation =
    confidence < 0.6 || (!!htmlLang && !!francLang && htmlLang !== francLang);
