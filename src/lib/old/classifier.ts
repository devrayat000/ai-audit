...
const RE_PRICE =
  /(?:\$|€|£|¥|₹|৳)\s?\d|(?:\d[\d,]*\s?(?:USD|EUR|GBP|INR|BDT|JPY))/i;
const RE_DATE =
  /\b(?:\d{1,2}[\/.-]\d{1,2}[\/.-]\d{2,4}|\d{4}-\d{2}-\d{2}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{1,2})/i;
const RE_LEGAL =
  /\b(allerg(?:y|en)|gluten|nut\s|peanut|warranty|disclaimer|liability|medical|diagnosis|treatment|prescription|interest rate|apr\b|return guaranteed|terms? of service|privacy policy|gdpr|consult\s+your\s+(doctor|physician))\b/i;
...
export function classifyTextNodes(
  html: string,
  ...
  const preserveTerms = new Set(
    glossary
      .filter(
        (g) => g.handling === "preserve" || g.handling === "transliterate",
      )
      .map((g) => g.source.toLowerCase()),
  );
  let counter = 0;
  ...
  function classify(
    node: Text,
    parent: Element,
  ): { c: TextClassification; hint: string } {
    const raw = node.data ?? "";
    ...
    if (
      tag === "title" ||
      (tag === "meta" && (parent.attribs?.name ?? "") === "description")
    ) {
      return { c: "transcreate", hint: "head" };
    }
    if (
      cls.includes("hero") ||
      cls.includes("tagline") ||
