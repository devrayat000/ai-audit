...
export class InMemoryTranslationCache implements TranslationCache {
  ...
  set(hash: string, value: string) { this.map.set(hash, value); }
}

export function hashText(s: string, mode: TextClassification): string {
  return crypto.createHash("sha256").update(`${mode}|${s}`).digest("hex").slice(0, 32);
}
...
function applyKeepTokens(text: string, glossarySources: string[]): { protected: string; tokens: Map<string, string> } {
...
function preservationCheck(source: string, target: string): { ok: boolean; reason?: string } {
  const numbersSrc = (source.match(/\b\d[\d,.]*\b/g) ?? []).filter((n) => n.length >= 2);
...
export interface TranslationResult {
  perNodeId: Map<string, { translation: string; classification: TextClassification; preservationOk: boolean; preservationReason?: string }>;
  errors: string[];
}

export async function translateClassified(input: TranslateBatchInput): Promise<TranslationResult> {
  const { client, cfg, cache, nodes } = input;
  const errors: string[] = [];
  const out = new Map<string, { translation: string; classification: TextClassification; preservationOk: boolean; preservationReason?: string }>();
  const glossarySources = cfg.glossary.filter((g) => g.handling !== "translate").map((g) => g.source);
  ...
  for (const n of [...groups.preserve, ...groups["preserve-pn"]]) {
    ...
    if (n.classification === "preserve-pn") {
      const entry = cfg.glossary.find((g) => g.source.toLowerCase() === n.text.toLowerCase());
      if (entry) {
    ...
    out.set(n.id, { translation, classification: n.classification, preservationOk: true });
  }
  ...
  for (const cls of ["literal", "transcreate", "flag-legal"] as const) {
    ...
    for (const n of group) {
      ...
      if (cached !== undefined) {
        ...
        out.set(n.id, { translation: restored, classification: cls, preservationOk: check.ok, preservationReason: check.reason });
      } else {
        uncached.push(n);
      }
    }
    ...
    if (!client) {
      ...
      errors.push("Anthropic client unavailable: kept source text for translatable strings.");
      continue;
    }

    const system =
      cls === "literal" ? SYSTEM_LITERAL(cfg) : cls === "transcreate" ? SYSTEM_TRANSCREATE(cfg) : SYSTEM_FLAG_LEGAL(cfg);
    ...
    for (const batch of chunk(uncached, MAX_PER_BATCH)) {
      ...
      try {
        ...
        const parsed = JSON.parse(json) as { id: string; translation: string }[];
        for (const item of parsed) {
          const n = batch.find((x) => x.id === item.id);
          if (!n) continue;
          const restored = restoreKeepTokens(item.translation, protectedMap.get(n.id)!.tokens);
          const check = preservationCheck(n.text, restored);
          const final = check.ok ? restored : n.text; // fail-closed
          out.set(n.id, { translation: final, classification: cls, preservationOk: check.ok, preservationReason: check.reason });
          cache.set(hashText(protectedMap.get(n.id)!.protected, cls), check.ok ? item.translation : n.text);
        }
        // any missing
        for (const n of batch) {
          if (!out.has(n.id)) {
            out.set(n.id, { translation: n.text, classification: cls, preservationOk: false, preservationReason: "Missing in LLM response" });
          }
        }
      } catch (e) {
        errors.push(`Batch failed (${cls}): ${e instanceof Error ? e.message : String(e)}`);
        for (const n of batch) {
          out.set(n.id, { translation: n.text, classification: cls, preservationOk: false, preservationReason: "LLM error" });
        }
      }
    }
  }
