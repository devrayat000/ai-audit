...
interface Props {
  ...
  currentPath: string;
  /** When true, nav stays transparent until scroll (hero pages). */
...
export function RestaurantNav({
  ...
  const onLight = !transparentTop || scrolled;
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-center justify-between h-16 md:h-20">
          <a href={links[0]?.href ?? "/"} className="flex flex-col leading-none group">
            <span
              className={cn(
            ...
            {links.map((link) => {
              ...
              return (
                <a
                  ...
                  href={link.href}
                  className={cn(
                    "font-sans text-sm tracking-widest uppercase transition-colors duration-200 pb-0.5 border-b-[1.5px]",
                    onLight ? "text-foreground" : "text-white",
                    isActive
                      ? onLight
                        ? "border-foreground"
        ...
        <nav className="flex flex-col px-6 py-4 gap-1">
          {links.map((link) => (
            <a
              ...
              className={cn(
                "font-sans text-sm tracking-widest uppercase py-3 border-b border-border last:border-0 transition-colors",
